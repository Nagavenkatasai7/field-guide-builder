...
import { z } from "zod";
import { setEngagementItemStatus, storageEnabled } from "@/lib/storage";
...
const Body = z.object({
  ...
  status: z.enum(["fresh", "used", "dismissed"]),
});
...
export async function POST(request: Request) {
  ...
  const ok = await setEngagementItemStatus(parsed.data.id, parsed.data.status);
